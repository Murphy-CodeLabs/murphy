...
const LazorKitWalletProviderClient = dynamic(
  () =>
    import("./client-lazorkit-provider").then(
      (mod) => mod.ClientLazorKitProvider
    ),
  {
    ssr: false,
  }
...
// Provider Component
export function LazorKitWalletProvider({
  children,
}: {
  children: React.ReactNode;
}) {
  return (
    <LazorKitWalletProviderClient>{children}</LazorKitWalletProviderClient>
  );
}
```
...
export function ClientLazorKitProvider({
  children,
}: {
  children: React.ReactNode;
}) {
...
<TutorialVideoDialog
  ...
  videoSrc="https://www.youtube.com/embed/dQw4w9WgXcQ?si=iGPw_RYxfAY6bCSo"
  thumbnailSrc="/images/thumbnails/tutorial-video-thumbnail.png"
