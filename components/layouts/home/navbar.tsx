  ...
  return (
    <NavigationMenu value={value} onValueChange={setValue} asChild>
      <header
        ...
        {...props}
        className={cn(
          "fixed left-1/2 top-(--fd-banner-height) z-40 box-content w-full max-w-fd-container -translate-x-1/2 border-fd-foreground/10 transition-colors lg:mt-2 lg:w-[calc(100%-1rem)] lg:rounded-2xl",
          (!isTransparent || value.length > 0) && "border backdrop-blur-lg",
          props.className
        )}
      >
        <NavigationMenuList
          className="flex h-14 w-full flex-row items-center px-4 lg:h-16"
          asChild
        >
